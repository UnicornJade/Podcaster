...
def crawler():
    ...
    if mode == '指定播客Url爬取':
        ...
        if st.checkbox("选择csvs所在文件夹"):
            st.caption("选择csvs所在文件夹")
            csvsdir = st.text_input('')
            download = st.button("Download Episodes")
            if csv and download:
                csv_files = glob.glob(os.path.join(csvsdir, '*.csv'))
                for csv_file in csv_files:
                    ep_names,urls,audio_dir = redirect_audio.read_csv_url(csv_file)
                    # 创建一个状态指示器
                    with st.spinner("Processing audio redirects..."):
                        # 调用函数，等待完成
                        redirect_audio.get_redirect(ep_names,urls,audio_dir)
